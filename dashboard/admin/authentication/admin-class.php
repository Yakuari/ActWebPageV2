<?php 
    ...
    class ADMIN
        ...
        public function sendotp($otp,$email){
            ...
            echo "<script>alert('We sent the OTP to $email.'); window.location.href = '../../../verify-otp';</script>";
        ...
        public function verifyOTP($username, $password, $otp, $tokencode, $email, $csrf_token){
            if($otp == $_SESSION['OTP']){
                ...
                $subject = "VERIFICATION SUCCES";
                $message = "
                <!DOCTYPE html>
                <html>
                <head>
                <meta charset = 'UTF=8'>
                <title> OTP Verification Succes</title>
                <style>
                body {
                font-family:
                background-color:
